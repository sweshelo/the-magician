...
export const SoundManagerContext = createContext<
  SoundManagerContextType | undefined
>(undefined);

export const SoundManagerProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  ...
  const [agentInterruptPlay, agentInterruptControls] = useSound(
    "/sound/se/agent-interrupt.ogg",
    { volume: 0.25 },
  );
  const [bindPlay, bindControls] = useSound("/sound/se/bind.ogg", {
    volume: 0.25,
  });
  const [blockPlay, blockControls] = useSound("/sound/se/block.ogg", {
    volume: 0.25,
  });
  const [bouncePlay, bounceControls] = useSound("/sound/se/bounce.ogg", {
    volume: 0.25,
  });
  const [clockUpFieldPlay, clockUpFieldControls] = useSound(
    "/sound/se/clock-up-field.ogg",
    { volume: 0.25 },
  );
  const [clockUpPlay, clockUpControls] = useSound("/sound/se/clock-up.ogg", {
    volume: 0.25,
  });
  const [copiedPlay, copiedControls] = useSound("/sound/se/copied.ogg", {
    volume: 0.25,
  });
  const [copyingPlay, copyingControls] = useSound("/sound/se/copying.ogg", {
    volume: 0.25,
  });
  const [cpConsumePlay, cpConsumeControls] = useSound(
    "/sound/se/cp-consume.ogg",
    { volume: 0.25 },
  );
  const [cpIncreasePlay, cpIncreaseControls] = useSound(
    "/sound/se/cp-increase.ogg",
    { volume: 0.25 },
  );
  const [damagePlay, damageControls] = useSound("/sound/se/damage.ogg", {
    volume: 0.25,
  });
  const [deactivePlay, deactiveControls] = useSound("/sound/se/deactive.ogg", {
    volume: 0.25,
  });
  const [deletedPlay, deletedControls] = useSound("/sound/se/deleted.ogg", {
    volume: 0.25,
  });
  const [drawPlay, drawControls] = useSound("/sound/se/draw.ogg", {
    volume: 0.25,
  });
  const [drivePlay, driveControls] = useSound("/sound/se/drive.ogg", {
    volume: 0.25,
  });
  const [effectPlay, effectControls] = useSound("/sound/se/effect.ogg", {
    volume: 0.25,
  });
  const [evolvePlay, evolveControls] = useSound("/sound/se/evolve.ogg", {
    volume: 0.25,
  });
  const [fortitudePlay, fortitudeControls] = useSound(
    "/sound/se/fortitude.ogg",
    { volume: 0.25 },
  );
  const [growPlay, growControls] = useSound("/sound/se/grow.ogg", {
    volume: 0.25,
  });
  const [guardPlay, guardControls] = useSound("/sound/se/guard.ogg", {
    volume: 0.25,
  });
  const [leavePlay, leaveControls] = useSound("/sound/se/leave.ogg", {
    volume: 0.25,
  });
  const [openPlay, openControls] = useSound("/sound/se/open-trash.ogg", {
    volume: 0.25,
  });
  const [oraclePlay, oracleControls] = useSound("/sound/se/oracle.ogg", {
    volume: 0.25,
  });
  const [overheatPlay, overheatControls] = useSound("/sound/se/overheat.ogg", {
    volume: 0.25,
  });
  const [penetratePlay, penetrateControls] = useSound(
    "/sound/se/penetrate.ogg",
    { volume: 0.25 },
  );
  const [purpleConsumePlay, purpleConsumeControls] = useSound(
    "/sound/se/purple-consume.ogg",
    { volume: 0.25 },
  );
  const [purpleIncreasePlay, purpleIncreaseControls] = useSound(
    "/sound/se/purple-increase.ogg",
    { volume: 0.25 },
  );
  const [rebootPlay, rebootControls] = useSound("/sound/se/reboot.ogg", {
    volume: 0.25,
  });
  const [recoverPlay, recoverControls] = useSound("/sound/se/recover.ogg", {
    volume: 0.25,
  });
  const [bangPlay, bangControls] = useSound("/sound/se/bang.ogg", {
    volume: 0.25,
  });
  const [silentPlay, silentControls] = useSound("/sound/se/silent.ogg", {
    volume: 0.25,
  });
  const [speedmovePlay, speedmoveControls] = useSound(
    "/sound/se/speedmove.ogg",
    { volume: 0.25 },
  );
  const [trashPlay, trashControls] = useSound("/sound/se/trash.ogg", {
    volume: 0.25,
  });
  const [triggerPlay, triggerControls] = useSound("/sound/se/trigger.ogg", {
    volume: 0.25,
  });
  const [unblockablePlay, unblockableControls] = useSound(
    "/sound/se/unblockable.ogg",
    { volume: 0.25 },
  );
  const [withdrawalPlay, withdrawalControls] = useSound(
    "/sound/se/withdrawal.ogg",
    { volume: 0.25 },
  );
  ...
  const soundMap = useMemo(
    () => ({
      "agent-interrupt": {
        ...
        controls: agentInterruptControls,
      },
      ...
      "purple-consume": {
        ...
        controls: purpleConsumeControls,
      },
      "purple-increase": {
        play: purpleIncreasePlay,
        controls: purpleIncreaseControls,
      },
  ...
  const playBgm = useCallback(async () => {
    ...
    try {
      ...
      const buffer = await (
        await fetch("/sound/bgm/Quiet Madness.wav")
      ).arrayBuffer();
...
export const useSoundManager = (): SoundManagerContextType => {
  ...
  if (context === undefined) {
    throw new Error(
      "useSoundManager must be used within a SoundManagerProvider",
    );
  }
  return context;
};
