...
export const useGame = () => {
  ...
  const setRound = (round: number): void =>
    dispatch({ type: "SET_ROUND", round });
  const setPlayer = (player: IPlayer): void =>
  ...
  const selfHand = useMemo<ICard[]>(
    () => (self?.hand ?? []) as ICard[],
    [self],
  );
  const selfDeck = useMemo<ICard[]>(
    () => (self?.deck ?? []) as ICard[],
    [self],
  );
  const selfField = useMemo<IUnit[]>(() => self?.field ?? [], [self]);
  const selfTrash = useMemo<ICard[]>(() => self?.trash ?? [], [self]);
  const selfTrigger = useMemo<ICard[]>(
    () => (self?.trigger ?? []) as ICard[],
    [self],
  );
  ...
  const opponent = useMemo<IPlayer | undefined>(
    () =>
      state.players &&
      Object.entries(state.players).find(([key]) => key !== selfPlayerId)?.[1],
    [state.players, selfPlayerId],
  );
  ...
  const opponentField = useMemo<IUnit[]>(
    () => opponent?.field ?? [],
    [opponent],
  );
  const opponentTrash = useMemo<ICard[]>(
    () => opponent?.trash ?? [],
    [opponent],
  );
  const opponentTrigger = useMemo<ICard[]>(
    () => (opponent?.trigger ?? []) as ICard[],
    [opponent],
  );
